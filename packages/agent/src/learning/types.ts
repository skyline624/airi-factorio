...
export interface Ops {
  ...
  /** Put a furnace ON the nearest drill's output tile so it's fed hands-free (automation rung 2 — the mod computes the drop tile). Use THIS to fix a drill reading `waiting_for_space_in_destination`. Idempotent; clears your own misplaced furnaces blocking the spot. Then fuel both with coal. e.g. `placeFurnaceAtDrill()`. PREFER this over placeAt for the drill->furnace feed. */
