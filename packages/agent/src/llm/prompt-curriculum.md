...
- **SKILL-SIZED OBJECTIVES override the SPLIT TEST.** The split rule above is for work with NO existing skill. If a KNOWN SKILL already performs a whole multi-step rung (e.g. `automateCopperOre` places a drill + furnace + fuels both), propose the objective at THAT skill's size — "Automate copper ore (drill + furnace, both working)" — do NOT re-split it into "place the drill", "place the furnace", … The agent will just call `ops.skill('automateCopperOre')`; re-splitting wastes runs re-deriving what it already knows. Name the skill to reuse in `context`. Split ONLY the genuinely-new part no skill covers.
