...
5. Always `await ops.walkToEntity(name, radius)` before mining/placing/transferring on a target, and check `ok` after every op. Use a GENEROUS `radius` (200+) — NEVER a small one like 10–20: you routinely wander off to mine, so a machine you placed earlier is then far away, and a small radius returns "not found" and wastes the whole attempt. When returning to your OWN drill/furnace to load it, always pass 200.
...
2. **Secure the inputs before acting.** Check `(await ops.getState()).inventory` actually holds every ingredient/item you need. If something is missing, craft or gather it FIRST (and look up ITS recipe too). Never call `craftItem`/`placeEntity` for something whose inputs you don't yet hold.
   - **Your smelted plates are in the FURNACE'S OUTPUT, not your hand.** Before you craft anything from iron/copper plates, COLLECT them: `await ops.walkToEntity('stone-furnace', 200)` then `await ops.moveItems({ item: 'iron-plate', entity: 'stone-furnace', toEntity: false })`. `productionStats` may say 28 plates were made while your inventory shows 0 — they're sitting in the furnace.
   - **To craft another machine you must MINE its raw inputs first.** A `stone-furnace` is 5 stone (mine stone), a `burner-mining-drill` needs iron gears (= iron plates, collect them from the furnace) + a stone-furnace, and every burner needs coal (mine a stock). Gather stone/coal/plates BEFORE `craftItem` — don't assume they're in your inventory.
...
4. **VERIFY, then FIX — and read the status correctly (this is the #1 mistake).** After placing/fueling, `await ops.scan()` and check EACH machine's `status`. Success = `working`. A machine that is NOT working is almost always **correctly placed but missing an input** — supply the input, do NOT move or rebuild it:
   ...
   - `waiting_for_space_in_destination` on a **drill** → it is mining FINE but has nowhere to output the ore. Call `await ops.placeFurnaceAtDrill()` (or place a belt/chest at its drop). This is the NEXT step toward automation — do NOT pick up or relocate the drill (a common, wasteful mistake).
...
- `await ops.walkToEntity(name, searchRadius?)` — walk to the nearest matching entity. Do this BEFORE mining/placing/moving on it. Pass a GENEROUS `searchRadius` (200+), especially to reach a machine you placed earlier after mining elsewhere — a small radius (10–20) returns "not found" and the step fails. e.g. `await ops.walkToEntity('stone-furnace', 200)`.
...
- `await ops.placeFurnaceAtDrill(furnaceName?)` — **use THIS to feed a drill into a furnace.** Puts a furnace ON the nearest drill's output tile (the exact geometry you keep getting wrong), idempotent, and clears your own misplaced furnaces blocking the spot. This is the fix for a drill stuck at `waiting_for_space_in_destination`. e.g. `await ops.placeFurnaceAtDrill()`, then fuel both with coal.
- `await ops.placeEntity(name)` — place one from your inventory with AUTO-SNAP. For a drill use `placeDrillOn(resource)`, for a furnace-on-a-drill use `placeFurnaceAtDrill()` — this is only for a standalone furnace/machine. e.g. `await ops.placeEntity('stone-furnace')`.
